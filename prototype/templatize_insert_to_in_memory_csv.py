    ...
    for table_name, attributes, is_both_entity_relationship_in_table, primary_keys in tables:
        if is_both_entity_relationship_in_table and entity_or_relationship_node.is_relationship():#folded relationship - relationship has to be 1:N
            if (check_if_folded_relationship_is_between_subclasses_and_all_subclasses_in_same_table(graph, entity_or_relationship_node, table_name) and
                ...
                temp_table_name, inserted_attribute_names, table_index_list = generate_update_statement(in_memory_csvs, entity_or_relationship_node, values_copy_2, index_mapping_copy, table_name, attributes, custom_types)
                ...
                if temp_table_name not in table_index_mapping_for_node:
                    table_index_mapping_for_node[temp_table_name] = (table_index_list, False)#index_list, is_mvd_table
                ...
                values_copy_2["attributes"] = values_copy
                temp_table_name, inserted_attribute_names, table_index_list = generate_update_statement(in_memory_csvs, entity_or_relationship_node, values_copy_2, index_mapping_updated_to_match_table_columns,
                                                                                      table_name, attributes, custom_types)
                if temp_table_name not in insert_table_attribute_names:
                    insert_table_attribute_names[temp_table_name] = inserted_attribute_names
                if temp_table_name not in table_index_mapping_for_node:
                    table_index_mapping_for_node[temp_table_name] = (table_index_list, False)
        else:
            ...
            if check_if_the_table_for_insert_own_mvd_attribute(graph, entity_or_relationship_node, table_name):
                ...
                for item in values_updated_to_match_table_columns[attributes[-1][0]]:#assume last entry is the mvd - there could be more than 1 pks but only 1 entry mvd at last
                    ...
                    inserted_attribute_names = generate_insert_statement_for_mvd_table(in_memory_csvs, values_copy,
                ...
                for item in values_updated_to_match_table_columns[attributes[-1][0]]:#assume last entry is the mvd - only 1 pk and only 1 entry mvd at last
                    ...
                    values_copy[attributes[-1][0]] = item
                    inserted_attribute_names = generate_insert_statement_for_mvd_table(in_memory_csvs, values_copy, table_name,
                ...
                inserted_attribute_names, table_index_list = generate_insert_statement_for_one_table(in_memory_csvs, values_copy, index_mapping_updated_to_match_table_columns_copy,
                                                                                   table_name, primary_keys, attributes, custom_types, entity_or_relationship_node.unique_name)
                if table_name not in insert_table_attribute_names:
                    insert_table_attribute_names[table_name] = inserted_attribute_names
                if table_name not in table_index_mapping_for_node:
                    table_index_mapping_for_node[table_name] = (table_index_list, False)
                ...
                temp_table_name, inserted_attribute_names, table_index_list  = generate_update_statement_for_folded_weak_entity(in_memory_csvs, values_updated_to_match_table_columns, index_mapping_updated_to_match_table_columns,
                ...
                inserted_attribute_names, table_index_list = generate_insert_statement_for_one_table(in_memory_csvs, values_updated_to_match_table_columns, index_mapping_updated_to_match_table_columns,
    ...
    for attr_name, attr_type, attr_unique_name, attr_entity_unique_name in attributes:#Instructor mapped to say Person, Instructor
        if attr_name in values:#table Instructor, insert query Instructor handled here
            ...
            if attr_type in custom_types:#composite
                temp_values.append(to_pg_composite(values[attr_name]))
            ...
            attribute_names.append(attr_name)
            table_index_mapping.append('null')
...
def generate_update_statement(in_memory_csvs, entity_or_relationship_node, values, index_mapping, table_name, attributes: List[Tuple[str, str]], custom_types: Dict[str, List[Tuple[str, str]]]):
    attribute_names = []
    temp_values = []
    table_index_mapping = []
    ...
    for attr_name, attr_type, attr_unique_name, attr_entity_unique_name in attributes:
        if attr_name in values["primary_keys"]:
            ...
            # Custom type without an array
            if attr_type in custom_types:#composite
                temp_values.append(to_pg_composite(values["attributes"][attr_name]))
                attribute_names.append(attr_name)
                table_index_mapping.append(index_mapping[attr_name])
            elif attr_type.endswith('[]'): #array attribute
                #doesn't handle composite type of array
...
def generate_update_statement_for_folded_weak_entity(in_memory_csvs, values, index_mapping, table_name, primary_keys, attributes: List[Tuple[str, str]], custom_types: Dict[str, List[Tuple[str, str]]],
...
def execute_templatized_insert(in_memory_csvs, entity_or_relationship_node:Node, values, tables_insert_index_mapping, node_index_to_attribute_mapping,
...
def aggregate_folded_weak_entity_by_table_pk(in_memory_csvs, unaggregated_data, weak_entity_attributes, table_name, table_primary_keys, weak_entity_unique_name):
    attribute_names = []
    attribute_names.extend(table_primary_keys)
    attribute_names.append(weak_entity_unique_name)
    weak_entity_data = unaggregated_data
