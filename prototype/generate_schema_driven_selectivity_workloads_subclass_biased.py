...
class ShapeGenerator:
    def __init__(
        ...
        *,
        ...
        self._strong_weights = tuple(
            subclass_weight if entity.is_subclass else 1.0
    ...
    def _choose_strong_entity(self, rng: random.Random) -> EntitySpec:
        if self.subclass_weight == 1.0:
            return rng.choice(self.catalog.strong_entities)
        return _weighted_choice(
            rng,
            self.catalog.strong_entities,
            self._strong_weights,
        )

    def _choose_relationship(self, rng: random.Random) -> RelationshipSpec:
        if self.hub_penalty == 0.0:
