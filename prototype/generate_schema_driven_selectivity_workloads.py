...
def build_catalog(graph, profile) -> ConceptualCatalog:
    ...
    for node in graph.nodes:
        ...
        owner = getattr(node, "parent_entity", None)
        entities[object_id] = EntitySpec(
            name=str(node.name),
            object_id=object_id,
            is_weak=bool(getattr(node, "is_weak_entity", False)),
            owner_id=(str(owner.unique_name).lower() if owner is not None and getattr(node, "is_weak_entity", False) else None),
...
class ShapeGenerator:
    def __init__(self, catalog: ConceptualCatalog):
        self.catalog = catalog

    def selection_projection(self, rng: random.Random) -> str:
        entity = rng.choice(self.catalog.strong_entities)
    ...
    def _relationship_from(self, relationship: RelationshipSpec):
        left = self.catalog.entities[relationship.left.entity_id]
        right = self.catalog.entities[relationship.right.entity_id]
        return (
    ...
    def relationship_join(self, rng: random.Random) -> str:
        relationship = rng.choice(self.catalog.relationships)
        from_sql, left, right = self._relationship_from(relationship)
        projected = [
    ...
    def complex_multi_join(self, rng: random.Random) -> str:
        path = rng.choice(self.catalog.relationship_paths)
        left = self.catalog.entities[path.left.entity_id]
        common = self.catalog.entities[path.first_common.entity_id]
        right = self.catalog.entities[path.right.entity_id]
        from_sql = (
...
def generate_schema_driven_workloads(
    ...
    *,
    ...
    shape_generator = ShapeGenerator(catalog)
...
def main():
    ...
    targets = tuple(float(value) for value in arguments.targets.split(","))
