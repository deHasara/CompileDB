...
def init_database(db_name, load_file, include_db_initialization_cost=False):
    ...
    start_search_for_schema_for_generated_workload(graph, include_db_initialization_cost)
    ...
    #load_workload_queries_node_costs_only(db_name, load_file, include_db_initialization_cost)#get estimated cost without exec time for workload - workload stat only - for synthetic experiments
    load_workload_queries(db_name, workload_file, include_db_initialization_cost)
...
def test_config(db_name, load_file, include_db_initialization_cost=False):
...
def validate_cost_model_for_configs(db_name, load_file, include_db_initialization_cost=False):
    create_database_if_not_exists(db_name)
    ...
    run_different_configurations(db_name, load_file, workload_file, graph, include_db_initialization_cost)
...
def run_progressively_increasing_iterative_greedy_algorithm(db_name, load_file, include_db_initialization_cost=False):
    create_database_if_not_exists(db_name)
    ...
    run_greedy_algorithm_for_increasing_no_of_iterations(db_name, load_file, workload_file, graph, include_db_initialization_cost)
...
def main():
    ...
    parser.add_argument("--include-db-init-cost", action="store_true", help="include DB initialization time for optimization cost")
    #argparse converts --include-db-init-cost to include_db_init_cost

    args = parser.parse_args()

    args.include_db_init_cost = False
    ...
    if args.command == "init" or args.command == "insert" or args.command == "test_config" or args.command == "run_different_configs" or args.command == "prog_greedy":
        ...
        if args.command == "init":
            init_database(args.db_name, args.load_file, args.include_db_init_cost)
            ...
            validate_cost_model_for_configs(args.db_name, args.load_file, args.include_db_init_cost)
        elif args.command == "prog_greedy":#execute greedy algorithm with number of iterations increasing progressively to explore
            run_progressively_increasing_iterative_greedy_algorithm(args.db_name, args.load_file, args.include_db_init_cost)                                                      #improved solutions over time.
        else:
