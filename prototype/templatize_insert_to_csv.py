        ...
        for attribute in node.attribute_list:
            if attribute.get("is_multivalued") and attribute.get("mapped_table") is not None and attribute.get("is_in_separate_table"):#mvd in separate table
                if attribute.get("entity_unique_name") != node.unique_name and attribute.get("mapped_table")[1] == table_name:#mvd came from parent node not from node itself
                    attribute_entity = graph.get_node_by_name(attribute.get("entity_unique_name"))
                    assert attribute_entity is not None
                    #assert attribute_entity.mapped_table is not None - relaxed the constraint to parent to have option to be no table and parent's mvd still be in separate table
                    assert attribute.get("mapped_table")[1] == table_name
                    return True
    return False
    ...
    for table_name, attributes, is_both_entity_relationship_in_table, primary_keys in tables:
        if is_both_entity_relationship_in_table and entity_or_relationship_node.is_relationship():#folded relationship - relationship has to be 1:N
            if (check_if_folded_relationship_is_between_subclasses_and_all_subclasses_in_same_table(graph, entity_or_relationship_node, table_name) and
                ...
                temp_table_name, inserted_attribute_names, table_index_list = generate_update_statement(entity_or_relationship_node, values_copy_2, index_mapping_copy, table_name, attributes, custom_types)
                ...
                values_copy_2["attributes"] = values_copy
                temp_table_name, inserted_attribute_names, table_index_list = generate_update_statement(entity_or_relationship_node, values_copy_2, index_mapping_updated_to_match_table_columns,
            ...
            if check_if_the_table_for_insert_own_mvd_attribute(graph, entity_or_relationship_node, table_name):
                ...
                for item in values_updated_to_match_table_columns[attributes[-1][0]]:#assume last entry is the mvd - there could be more than 1 pks but only 1 entry mvd at last
                    ...
                    inserted_attribute_names = generate_insert_statement_for_mvd_table(values_copy,
                ...
                for item in values_updated_to_match_table_columns[attributes[-1][0]]:#assume last entry is the mvd - only 1 pk and only 1 entry mvd at last
                    ...
                    values_copy[attributes[-1][0]] = item
                    inserted_attribute_names = generate_insert_statement_for_mvd_table(values_copy, table_name,
                ...
                temp_table_name, inserted_attribute_names, table_index_list  = generate_update_statement_for_folded_weak_entity(values_updated_to_match_table_columns, index_mapping_updated_to_match_table_columns,
...
#Batch insert - insert to csv for batch insert
...
def generate_update_statement(entity_or_relationship_node, values, index_mapping, table_name, attributes: List[Tuple[str, str]], custom_types: Dict[str, List[Tuple[str, str]]]):
    attribute_names = []
    temp_values = []
    table_index_mapping = []
    ...
    if temp_values:
        temp_table_name = "temp_" + entity_or_relationship_node.unique_name#multiple relationships can be folded in table - different attribute sets - need to identify by folded relationship node
...
def write_to_csv(table_name, values):
    ...
    if wid is None:#worker id not initialized - no parallelization
        ...
        with open('data/'+table_name+'_'+str(wid)+'.csv', 'a', newline='') as f:
            writer = csv.writer(f, quoting=csv.QUOTE_MINIMAL)
            writer.writerow(values)
