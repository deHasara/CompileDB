...
def generate_insert_statements_in_batch(entity_or_relationship_node:Node, values, tables: List[Tuple[str, List[Tuple[str, str, str]]]], custom_types: Dict[str, List[Tuple[str, str]]],
                                        graph:Graph, insert_table_attribute_names:Dict, in_memory_csvs:List) -> List[str]:
    ...
    for table_name, attributes, is_both_entity_relationship_in_table, primary_keys in tables:
        if is_both_entity_relationship_in_table and entity_or_relationship_node.is_relationship():#folded relationship - relationship has to be 1:N
            if (check_if_folded_relationship_is_between_subclasses_and_all_subclasses_in_same_table(graph, entity_or_relationship_node, table_name) and
                ...
                temp_table_name, inserted_attribute_names = generate_update_statement(in_memory_csvs, entity_or_relationship_node, values_copy_2, table_name, attributes, custom_types)
            ...
            if check_if_the_table_for_insert_own_mvd_attribute(graph, entity_or_relationship_node, table_name):
                for item in values_updated_to_match_table_columns[attributes[-1][0]]:#assume last entry is the mvd - there could be more than 1 pks but only 1 entry mvd at last
                    ...
                    inserted_attribute_names = generate_insert_statement_for_one_table(in_memory_csvs, values_copy, table_name, primary_keys, attributes, custom_types)
                ...
                temp_table_name, inserted_attribute_names  = generate_update_statement_for_folded_weak_entity(in_memory_csvs, values_updated_to_match_table_columns, table_name, primary_keys,
                                                                                           attributes, custom_types, entity_or_relationship_node)
                if temp_table_name not in insert_table_attribute_names:
                    insert_table_attribute_names[temp_table_name] = inserted_attribute_names

            else:
                inserted_attribute_names = generate_insert_statement_for_one_table(in_memory_csvs, values_updated_to_match_table_columns, table_name, primary_keys, attributes, custom_types,
                                                                                   entity_or_relationship_node.unique_name, graph)
                if table_name not in insert_table_attribute_names:
                    insert_table_attribute_names[table_name] = inserted_attribute_names

#Batch insert - insert to in-memory csv for batch insert
def generate_insert_statement_for_one_table(in_memory_csvs:List, values, table_name, primary_keys, attributes: List[Tuple[str, str]], custom_types: Dict[str, List[Tuple[str, str]]],
...
def generate_update_statement(in_memory_csvs, entity_or_relationship_node, values, table_name, attributes: List[Tuple[str, str]], custom_types: Dict[str, List[Tuple[str, str]]]):
    attribute_names = []
    temp_values = []
...
def generate_update_statement_for_folded_weak_entity(in_memory_csvs, values, table_name, primary_keys, attributes: List[Tuple[str, str]], custom_types: Dict[str, List[Tuple[str, str]]],
